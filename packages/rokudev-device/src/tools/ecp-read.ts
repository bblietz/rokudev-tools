...
registerToolsModule((tools) => {
  ...
  tools.set(
    ...
    tool({
      name: 'ecp_active_app',
      description:
        'Get the currently active channel via ECP /query/active-app from the targeted Roku.',
      inputSchema: { type: 'object', properties: { ...baseProps }, additionalProperties: false },
      handler: async (a) => {
        const t = await resolveTarget(a as Record<string, string>);
        await checkReachable(t.device, a['force'] === true);
  ...
  tools.set(
    ...
    tool({
      name: 'ecp_icon',
      description:
        'Fetch a channel icon as base64 from ECP /query/icon/<app_id> on the targeted Roku.',
      inputSchema: {
        type: 'object',
