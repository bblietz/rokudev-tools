...
function mkdirTmp() {
  return join(tmpdir(), `brs-gen-write-${randomUUID()}`);
}
...
describe('writeProject', () => {
  ...
  it('refuses to overwrite existing output_dir without flag', async () => {
    ...
    await expect(
      writeProject({ outputDir: out, files: sample, overwrite: false }),
  ...
  it('cleans up tmp dir when a write error occurs', async () => {
    ...
    // survives.
    await expect(
      writeProject({ outputDir: out, files: badFiles, overwrite: false }),
    ).rejects.toBeDefined();
