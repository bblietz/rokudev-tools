...
export class DevPortalInspect {
  ...
  async genkey(): Promise<{ ok: true; dev_id: string; key: string; raw_html: string; duration_ms: number }> {
    const start = Date.now();
    const boundary = buildBoundary();
    const body = buildMultipart(
    ...
    const idMatch = text.match(/Dev\s*ID[^:]*:\s*<[^>]*>\s*([0-9a-f]+)/i)
                ?? text.match(/Dev\s*ID[^:]*:\s*([0-9a-f]+)/i);
    const keyMatch = text.match(/Dev\s*Key[^:]*:\s*<[^>]*>\s*([0-9a-f]+)/i)
                 ?? text.match(/Dev\s*Key[^:]*:\s*([0-9a-f]+)/i);
    if (!idMatch || !keyMatch) {
      throw fail('GENKEY_FAILED', 'could not parse Dev ID / Dev Key from response',
        { excerpt: text.slice(0, 400) });
    }
    return { ok: true, dev_id: idMatch[1]!, key: keyMatch[1]!, raw_html: text, duration_ms: Date.now() - start };
  }

  async rekey(signedPkgPath: string, signingPassword: string): Promise<{ ok: true; message: string; duration_ms: number }> {
    const start = Date.now();
    let pkgBytes: Buffer;
    try { pkgBytes = await readFile(signedPkgPath); }
    catch { throw fail('REKEY_FAILED', `signed package not found: ${signedPkgPath}`); }
    const boundary = buildBoundary();
    const body = buildMultipart(
      [{ kind: 'field', name: 'mysubmit', value: 'Rekey' },
       { kind: 'field', name: 'passwd', value: signingPassword },
       { kind: 'file', name: 'archive', filename: basename(signedPkgPath),
         contentType: 'application/octet-stream', body: pkgBytes }],
      boundary,
    );
    const r = await digestRequest({
      method: 'POST',
  ...
  async packSigned(signingPassword: string): Promise<{ ok: true; pkg_bytes: Buffer; duration_ms: number }> {
    const start = Date.now();
    const boundary = buildBoundary();
    const body = buildMultipart(
    ...
    if (!m) throw fail('PACKAGE_FAILED', 'no .pkg link in response',
      { excerpt: r1.bodyText.slice(0, 400) });
  ...
  async queryRegistry(devId: string): Promise<{ ok: true; registry: unknown; duration_ms: number }> {
    const start = Date.now();
    const r = await digestRequest({
      method: 'GET',
    ...
    if (r.statusCode === 401) throw fail('DEVICE_AUTH_FAILED', 'dev portal rejected credentials');
    if (r.statusCode !== 200) throw fail('DEVICE_UNREACHABLE', `registry query returned ${r.statusCode}`);
    return { ok: true, registry: parseXml(r.bodyText), duration_ms: Date.now() - start };
  }

  async profilerSnapshot(): Promise<{ ok: true; sections: Record<string, string>; raw_html_excerpt: string; truncated: boolean; duration_ms: number }> {
    const start = Date.now();
    const boundary = buildBoundary();
    const body = buildMultipart(
  ...
  async crashlogPull(): Promise<{ ok: true; log_text: string; truncated: boolean; duration_ms: number }> {
    const start = Date.now();
    const r = await digestRequest({
      method: 'GET',
    ...
    if (r.statusCode !== 200) throw fail('DEVICE_UNREACHABLE', `factory log returned ${r.statusCode}`);
    const MAX = 1024 * 1024;
