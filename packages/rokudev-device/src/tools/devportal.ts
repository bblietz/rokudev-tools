...
async function ensurePassword(args: Record<string, unknown>): Promise<{ device?: string; host: string; dev_password: string }> {
...
async function screenshotHandler(a: Record<string, unknown>): Promise<unknown> {
  ...
  return { ok: true, host: t.host, mime: shot.mime, bytes: shot.bytes, path, duration_ms: shot.duration_ms };
}
...
async function packSignedHandler(a: Record<string, unknown>): Promise<unknown> {
  ...
  return { ok: true, host: t.host, output_pkg: outputPkg, bytes: result.pkg_bytes.length, duration_ms: result.duration_ms };
}
...
registerToolsModule((tools) => {
  tools.set('sideload', tool({
    ...
    handler: async (a) => {
      const t = await ensurePassword(a);
      const { ok: _ok, ...r } = await new DevPortal(t.host, t.dev_password).sideload(a['zip_path'] as string);
      return { ok: true, host: t.host, ...r };
    },
  }));

  tools.set('unload', tool({
    name: 'unload',
  ...
  tools.set('diff_installed', tool({
    name: 'diff_installed',
    description: 'Compare a local project dir against the dev package currently installed on the Roku.',
    inputSchema: {
      type: 'object',
    ...
    handler: async (a) => {
      const t = await ensurePassword(a);
      const { ok: _ok, ...r } = await diffInstalled(t.host, t.dev_password, a['project_dir'] as string);
      return { ok: true, host: t.host, ...r };
    },
  }));

  tools.set('query_registry', tool({
    name: 'query_registry',
    ...
    handler: async (a) => {
      const t = await ensurePassword(a);
      const { ok: _ok, ...r } = await new DevPortalInspect(t.host, t.dev_password).queryRegistry(a['dev_id'] as string);
      return { ok: true, host: t.host, ...r };
    },
  }));

  tools.set('profiler_snapshot', tool({
    ...
    handler: async (a) => {
      const t = await ensurePassword(a);
      const { ok: _ok, ...r } = await new DevPortalInspect(t.host, t.dev_password).profilerSnapshot();
      return { ok: true, host: t.host, ...r };
    },
  }));

  tools.set('crashlog_pull', tool({
    name: 'crashlog_pull',
