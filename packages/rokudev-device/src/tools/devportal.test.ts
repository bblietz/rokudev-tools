...
function mockDevPortal(
  overrides: Partial<{ sideload: ReturnType<typeof vi.fn>; unload: ReturnType<typeof vi.fn> }> = {},
): void {
...
function mockDevPortalInspect(
  overrides: Partial<Record<string, ReturnType<typeof vi.fn>>> = {},
): void {
  mocks.DevPortalInspect.mockImplementation(() => ({
    screenshot: vi
      .fn()
      .mockResolvedValue({ mime: 'image/jpeg', bytes: 1234, base64: 'YWFh', duration_ms: 50 }),
    genkey: vi.fn().mockResolvedValue({
      ok: true,
      dev_id: 'aabbcc112233',
      key: 'keydata',
      raw_html: '<html/>',
    ...
    packSigned: vi
      .fn()
      .mockResolvedValue({ ok: true, pkg_bytes: Buffer.from('fakepkgdata'), duration_ms: 80 }),
    queryRegistry: vi
      .fn()
      .mockResolvedValue({ ok: true, registry: { section: {} }, duration_ms: 40 }),
    profilerSnapshot: vi.fn().mockResolvedValue({
      ok: true,
      sections: { Memory: 'usage data' },
      raw_html_excerpt: '<html/>',
      truncated: false,
      duration_ms: 60,
    }),
    crashlogPull: vi.fn().mockResolvedValue({
      ok: true,
      log_text: 'crash log text',
      truncated: false,
      duration_ms: 35,
    }),
    ...overrides,
...
describe('sideload happy path', () => {
  it('calls DevPortal.sideload(zip_path) and returns { ok, host, ... }', async () => {
    const sideloadFn = vi
      .fn()
      .mockResolvedValue({ message: 'Success', http_code: 200, duration_ms: 120 });
    mockDevPortal({ sideload: sideloadFn });

    const result = (await call('sideload', {
      host: '192.168.1.100',
      dev_password: 'devpw',
...
describe('unload happy path', () => {
  it('calls DevPortal.unload() and returns { ok, host, ... }', async () => {
    ...
    const result = (await call('unload', {
      host: '192.168.1.100',
      dev_password: 'devpw',
    })) as Record<string, unknown>;
...
describe('screenshot happy path', () => {
  it('calls DevPortalInspect.screenshot() with no args and returns inline by default', async () => {
    const screenshotFn = vi
      .fn()
      .mockResolvedValue({ mime: 'image/jpeg', bytes: 2048, base64: 'abc123', duration_ms: 75 });
...
describe('screenshot ref mode', () => {
  it('writes screenshot to ref path with mode 0o600', async () => {
    ...
    expect(path).toMatch(
      new RegExp(`${tmpDir.replace(/\//g, '/')}/.cache/rokudev/screenshots/[0-9a-f]{64}\\.jpg$`),
    );
...
describe('genkey happy path', () => {
  it('calls DevPortalInspect.genkey() and returns { ok, host, dev_id, key }', async () => {
    const genkeyFn = vi.fn().mockResolvedValue({
      ok: true,
      dev_id: 'deadbeef01234',
      key: 'keyABCD',
      raw_html: '<html/>',
      duration_ms: 50,
    });
    ...
    const result = (await call('genkey', {
      host: '192.168.1.100',
      dev_password: 'devpw',
    })) as Record<string, unknown>;
...
describe('pack_signed happy path', () => {
  it('calls DevPortalInspect.packSigned(), writes pkg to disk, returns { ok, host, output_pkg, bytes }', async () => {
    ...
    const packSignedFn = vi
      .fn()
      .mockResolvedValue({ ok: true, pkg_bytes: pkgData, duration_ms: 80 });
...
describe('query_registry happy path', () => {
  it('calls DevPortalInspect.queryRegistry(dev_id) and returns { ok, host, registry }', async () => {
    const queryFn = vi
      .fn()
      .mockResolvedValue({ ok: true, registry: { section1: { key: 'val' } }, duration_ms: 40 });
...
describe('profiler_snapshot happy path', () => {
  it('calls DevPortalInspect.profilerSnapshot() and returns { ok, host, sections }', async () => {
    const profilerFn = vi.fn().mockResolvedValue({
      ok: true,
      sections: { Memory: '4MB' },
      raw_html_excerpt: '<html/>',
      truncated: false,
    ...
    const result = (await call('profiler_snapshot', {
      host: '192.168.1.100',
      dev_password: 'devpw',
    })) as Record<string, unknown>;
...
describe('crashlog_pull happy path', () => {
  it('calls DevPortalInspect.crashlogPull() and returns { ok, host, log_text }', async () => {
    const crashlogFn = vi.fn().mockResolvedValue({
      ok: true,
      log_text: 'crash data here',
      truncated: false,
      duration_ms: 35,
    });
    mockDevPortalInspect({ crashlogPull: crashlogFn });

    const result = (await call('crashlog_pull', {
      host: '192.168.1.100',
      dev_password: 'devpw',
    })) as Record<string, unknown>;
...
describe('DEVICE_NO_PASSWORD', () => {
  it('throws DEVICE_NO_PASSWORD when no dev_password in args, env, or registry', async () => {
    ...
    await expect(call('sideload', { host: '127.0.0.1', zip_path: '/x.zip' })).rejects.toMatchObject(
      { ok: false, code: 'DEVICE_NO_PASSWORD', stage: 'device' },
    );
  });

  it('throws DEVICE_NO_PASSWORD for screenshot when password missing', async () => {
    mockDevPortalInspect();
    await expect(call('screenshot', { host: '127.0.0.1' })).rejects.toMatchObject({
      ok: false,
...
describe('pass-through failures', () => {
  it('sideload propagates SIDELOAD_REJECTED failure unwrapped', async () => {
    ...
    await expect(
      call('sideload', { host: '192.168.1.100', dev_password: 'pw', zip_path: '/tmp/app.zip' }),
  ...
  it('genkey propagates GENKEY_FAILED failure unwrapped', async () => {
    ...
    await expect(
      call('genkey', { host: '192.168.1.100', dev_password: 'pw' }),
